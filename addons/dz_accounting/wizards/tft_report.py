...
class BankBookWizard(models.TransientModel):
    ...
    def check_report(self):
        ...
        tft = {'current': self.env['account.liasse.config'].get_tft_financier_lines(self.date_from, self.date_to)}
        tft_prev = {'previous': self.env['account.liasse.config'].get_tft_financier_lines(self.date_from.replace(year=self.date_from.year - 1), self.date_to.replace(year=self.date_to.year - 1))}
        data.update(tft)
        data.update(tft_prev)
        return self.env.ref('dz_accounting.flux_tresorerie_report_action').report_action(self, data=data)
